...
ENV HOME=/root
ENV NUCLEAR_DATA_PATH=/nuclear_data
ENV OPENMC_CROSS_SECTIONS=/nuclear_data/nndc_hdf5/cross_sections.xml
ENV DATASETS=/datasets

# Download cross sections (NNDC and WMP) and ENDF data needed by test suite
# RUN ${HOME}/OpenMC/openmc/tools/ci/download-xs.sh
COPY download-xs.sh /root/OpenMC/openmc/tools/ci/download-xs.sh

ENV JUPYTER_CONFIG_DIR=/config/jupyter

# Install Jupyter Lab and start it with the container
